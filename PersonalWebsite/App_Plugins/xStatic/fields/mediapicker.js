...
angular.module('umbraco').controller("Umbraco.PropertyEditors.MediaPickerControllerSam",
    function ($scope, entityResource, mediaHelper, $timeout, userService, localizationService, editorService, overlayService, clipboardService) {
        ...
        var multiPicker = $scope.property.config.multiPicker && $scope.property.config.multiPicker !== '0' ? true : false;
        var onlyImages = $scope.property.config.onlyImages && $scope.property.config.onlyImages !== '0' ? true : false;
        var disableFolderSelect = $scope.property.config.disableFolderSelect && $scope.property.config.disableFolderSelect !== '0' ? true : false;
                ...
                entityResource.getByIds(ids, "Media").then(function (medias) {
                    ...
                    medias = ids.map(id => {
                        ...
                        var mediaItem = found ||
                        {
                            ...
                            id: $scope.property.config.idType !== "udi" ? id : null,
                            udi: $scope.property.config.idType === "udi" ? id : null,
                            icon: "icon-picture",
                            thumbnail: null,
                            trashed: true
                        };
        ...
        function appendMedia(media) {
            ...
            if ($scope.property.config.idType === "udi") {
        ...
        function sync() {
            $scope.property.value = selectedIds.join();
            ...
            localizationService.localizeMany(labelKeys)
                .then(function (data) {
                    ...
                    userService.getCurrentUser().then(function (userData) {
                        ...
                        if (!$scope.property.config.startNodeId) {
                            if ($scope.property.config.ignoreUserStartNodes === true) {
                                $scope.property.config.startNodeId = -1;
                                $scope.property.config.startNodeIsVirtual = true;
                            }
                            else {
                                $scope.property.config.startNodeId = userData.startMediaIds.length !== 1 ? -1 : userData.startMediaIds[0];
                                $scope.property.config.startNodeIsVirtual = userData.startMediaIds.length !== 1;
                            }
                        }
        ...
        function copyAllEntries() {
            if ($scope.mediaItems.length > 0) {
                ...
                localizationService.localize("clipboard_labelForArrayOfItems", [$scope.property.label]).then(function (localizedLabel) {
                    clipboardService.copyArray(clipboardService.TYPES.MEDIA, aliases, data, localizedLabel, "icon-thumbnail-list", $scope.property.id);
                });
            }
        }
        ...
        function add() {
            var mediaPicker = {
                startNodeId: $scope.property.config.startNodeId,
                startNodeIsVirtual: $scope.property.config.startNodeIsVirtual,
        ...
        $scope.sortableOptions = {
            ...
            update: function () {
                ...
                $timeout(function () {
                    ...
                    selectedIds = vm.mediaItems.map(media => $scope.property.config.idType === "udi" ? media.udi : media.id);
